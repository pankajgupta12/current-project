...
<?php  
//echo '<pre>'; print_r($_SESSION['reclean_email_reply']);
$client_name = get_sql("quote_new","name","where booking_id=".$_SESSION['reclean_email_reply']['job_id']);
?>
			    ...
			    <form class="form-horizontal" method="post" id="reclen_new_message" enctype="multipart/form-data">
		            <div class="mail-body replay_body">	
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <div class="col-sm-9">
							   <input type="text" class="text-left form-control" value="<?php echo $_SESSION['reclean_email_reply']['email_reply_to']; ?>" id="getemail" autocomplete="off" name="to_email" id="to_email">
							</div>
						...
						<div class="form-group col-md-12">
                            ...
                            <div class="col-sm-11">
							   <input type="text" name="bcic_subject" id="bcic_subject" autocomplete="off" class="text-left form-control" value="<?php echo 'Re-Clean reply for '.$_SESSION['reclean_email_reply']['job_id'];  ?>">
						    </div>
                        </div>
					   ...
					   <div class="form-group col-md-12 mail-chosse-file mail-chosse-file-new">
						...
						<input type="hidden" id="sent_new_msg_type" name="sent_new_msg_type" value="<?php echo $_SESSION['reclean_email_reply']['mail_type']; ?>">
						<input type="hidden" id="email_msgno" name="email_msgno" value="<?php echo $_SESSION['reclean_email_reply']['email_msgno']; ?>">
						<input type="hidden" id="reclean_email_reply" name="reclean_email_reply" value="email_reply">
						<input type="hidden" id="email_id" name="email_id" value="<?php echo $_SESSION['reclean_email_reply']['id']; ?>">
                    </div>
						...
						<textarea id="message_body" class="summernote" name="message_body">
							...
							<p></p>
							<p></p>
							<p>Hello <?php echo $client_name;  ?>, <br/>
							Thank you for your email. <br/>
							Your list has been forwarded to the cleaner who will arrange time and access directly with your Real Estate to attend the Reclean. </p><br/>	
							<p></p>
							<p></p>
							<?php include('signature.php'); ?>		
							<p></p>
							<?php echo $_SESSION['reclean_email_reply']['email_body']; ?>
						</textarea>
						
							<ul class="email_replay_list_box">	
								   <?php 				
											$getAttchment1 = mysql_query("SELECT *  FROM `bcic_email_attach` WHERE email_id = ".$_SESSION['reclean_email_reply']['id']." AND mail_type = '".$_SESSION['reclean_email_reply']['mail_type']."'");
