...
ini_set('memory_limit', '-1'); 
    ...
    if(isset($_POST)){
     ...
     // echo $_SERVER['DOCUMENT_ROOT'].'/mail/mail_attachments/'.$email_folder[$key2].'/'.$attchfile;						
	...
	//die;		
			...
			try {
					...
					$mail->AddReplyTo($to, 'Forward Message');
					$mail->AddAddress($to);
					...
					if(!$mail->Send())
					{
						...
						$allEmailsType  = mysql_query("SELECT *  FROM `email_config` WHERE `status` = 0");	
							...
							if($email_id != '' && $email_id != 0) {							
							  $bool = mysql_query("UPDATE `bcic_email` SET `last_activity` = '2' , last_activity_date = '".date('Y-m-d H:i:s')."' WHERE `id` = ".$email_id."");
							}
