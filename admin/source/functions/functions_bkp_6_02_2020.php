		...
		if ($r['description']==""){ 
			...
			if($r['job_type_id']=="1"){ 
				 if($r['bed']>0){ $desc.=' '.$r['bed'].' Beds,'; }
				  ...
				  if($r['blinds_type']!=""){ $desc.=' '.ucwords(str_replace("_"," ",$r['blinds_type'])).', '; }
				 ...
				 if($r['pest_flee']>0){ $desc.= ' & Flea and Tick'; }			
				  ...
				  if($r['bed']>0){ $desc.=' '.$r['bed'].' Beds,'; }
				  if($r['study']>0){ $desc.=' '.$r['study'].' Study,'; }
				  if($r['lounge_hall']>0){ $desc.=' '.$r['lounge_hall'].' lounge hall,'; }
...
function add_application_notes($appl_id,$heading,$comment , $customDate = null, $customNameForcx = null , $adminId = null,$staff_id = null){
...
function add_task_manager($task_id , $quote_id  , $task_type , $fallow_date , $fallow_time ,$response_type , $taskmanagerid , $job_id = 0){
...
function add_site_notifications($notificationArrayData)
{
...
function ready_desc_text($txt){
	...
	$phone_array = array("04","02"," 03"," 07"," 08"," +61"," 1300","55");
...
function fdate($num,$dd,$mm,$yy,$start_year,$end_year){
									$mm_arr=array("Jan","Feb","Mar","Apr","May","Jun","July","Aug","Sep","Oct","Nov","Dec");
									//echo $dd."-".$mm."-".$yy."<br>";
									$str="
									<select class='date_select' name=\"dd_$num\">";
										for ($i=1;$i<=31;$i++){
											if ($dd==$i){ $sel_dd="Selected"; }else{ $sel_dd=""; }
										...
										for ($i=0;$i<12;$i++){ 
											if ($mm==$i+1){$mm_sel="Selected"; }else{ $mm_sel=""; }
											$ii=$i+1;
										...
										while($from_year<$to_year){ 
											...
											if ($yy==$from_year){$yy_sel="Selected"; }else{ $yy_sel=""; }
...
function create_dd($field_name, $table,$id_field,$name_field,$cond = " 1 ",$onchng,$details,$field_id = null , $all_d = null){	
	...
	if($field_id == 'field_id_time') {
	 ...
	 if ($typeID == 31){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 96){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 33){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 59){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 60){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 58){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 34){ $str .= "id=". $field_name."_".$details['id'] . " ";  }elseif ($typeID == 91){ $str .= "id=". $field_name."_".$details['id'] . " ";  }else { $str .=  "id = ". $field_name;}
	...
	if (explode("=",$cond)[1] != 32 && explode("=",$cond)[1] != 41 && explode("=",$cond)[1] != 48 && explode("=",$cond)[1] != 49 && explode("=",$cond)[1] != 51 && explode("=",$cond)[1] != 53 && explode("=",$cond)[1] != 54 && ($table !="quote_for_option")  ) {
		   ...
		   if(explode("=",$cond)[1] == 21  || ($field_name == 'online_payment_status')) {
...
function get_field_value_nonhtml($details,$field){
	//echo $field;
	if ($_POST[$field]!=""){
		return htmlspecialchars($_POST[$field]); break;
	}else if ($_GET[$field]!=""){
		return $_GET[$field]; break;
	}else{ 
		return htmlspecialchars($details[$field]); break;
	} 
}
...
function sendmail($sendto_name,$sendto_email,$sendto_subject,$sendto_message,$replyto,$site_id , $quotefor = null) {
	//global $siteinfo;
...
function sendmailwithattachinvoce($sendto_name,$sendto_email,$sendto_subject,$sendto_message,$replyto,$site_id,$quote_id , $quotefor = null , $real_estate = null, $jobinv = null) {
...
";
		...
		ini_set('sendmail_from', $site['email']);
		mail($sendto_email,$sendto_subject,$email_message,$headers);	
...
function Staffsendmailwithattachinvoce ($sendto_name,$sendto_email,$sendto_subject,$sendto_message,$replyto, $staff_id)  {
...
";	
	    	...
	    	//$fileatt_name = "https://www.beta.bcic.com.au/admin/staff_agreement/".$filename;
...
function add_new_inv($payment_gateway_id){
	if($payment_gateway_id!=""){
		...
		$eol="<br>";
		  ...
		  $to="";
		  if($users['company_name']!=""){ $to = $users['company_name'].",".$eol; }
		  $to.=$users['name'].$eol;
...
function get_field_value($details,$field){									//echo $field;
	if ($_POST[$field]!=""){
		return $_POST[$field]; break;
	}else if ($_GET[$field]!=""){
		return $_GET[$field]; break;
	}else{ 
		return $details[$field]; break;
	} 
}
...
function checkphonenumber($arg){
   ...
   function sendmember_quotemail($quote_id,$sendemail){
			...
			$desc = "";
        		if($r['job_type_id']=="1"){ 
        				 if($r['bed']>0){ $desc.=' '.$r['bed'].' Beds,'; }
        				  ...
        				  if($r['furnished']=="Yes"){ $desc.=' Furnished ,'; }
        				  if($r['blinds_type']!=""){ $desc.=' '.ucwords(str_replace("_"," ",$r['blinds_type'])).' '; }
        				  if($r['property_type']=="1"){ $desc.= $r['property_type'] ; }
	...
	if($ins){ 
		...
		if($countre > 0) {
			...
			mysql_query("update sales_system set job_id= ".$booking_id." where id=".mysql_real_escape_string($sid)."");	
			 ...
			 $call_she  = mysql_query("insert into sales_system set quote_id='".($quote_id)."', job_id = ".$booking_id." ,  staff_name='".$staffname."', site_id=".$quote['site_id'].", task_manage_id='0' , status=1,  task_type='Client' ,  createOn='".date('Y-m-d H:i:s')."'");
		...
		$call_she1  = mysql_query("insert into sales_task_track set quote_id='".($quote_id)."', job_id = ".$booking_id." , staff_name='".$staffname."', admin_id='0',site_id=".$quote['site_id'].",stages=3, status=1, fallow_date='".date('Y-m-d H:i:s')."' ,fallow_created_date='".date('Y-m-d')."' ,task_manage_id='0' , task_type='Client' ,  fallow_time='".$fallow_time."' ,  task_status='2' , track_type = '2' , sales_task_id = '".$sid."' ,  createOn='".date('Y-m-d H:i:s')."'");
		...
		mysql_query("INSERT INTO `task_manager` (`fallow_date`, `fallow_time`, `completed_date`, `admin_id`, `job_id` , `task_type`, `quote_id`, `response_type`, `task_id`, `created_date`, `status`) VALUES ('".$fallow_date."', '".$fallow_time."','".date('Y-m-d H:i:s')."', '0', ".$booking_id." ,  '2', '".$quote_id."',  '15', '".$sid1."', '".date('Y-m-d H:i:s')."', '0');");	
	    ...
	    if($quoteid1 != '') {
		  ...
		  if(!empty($quotedetails)>0){ 
		   ...
		   $invoice_status = get_rs_value("jobs","invoice_status",$job_id);
			  if($invoice_status=="1"){ $invoice_status = "<strong>Paid</strong>"; }else{ $invoice_status = "<strong>Pending</strong>"; }	
		 ...
		 if($staff['staff_gst'] == 1) {
		     $gst =  '<tr>
			  ...
			  <tbody>
				...
				<tr>
				  <td>'.$quote_for_option['company_name_member_sites'].'
			...
			<span style="color:#00b8d4;font-weight:300;padding-bottom:10px;"> '.$quote_for_option['booking_email'].' </span><br>
		 ...
		 if($br_in_bcic == 1) {
			 ...
			 $url = $siteUrl1."/members/".$qu_id."/inventory"; 
				...
				$html2.= '<tr><td class="listing1" style="list-style="none;"><style>.listing1 ul li { list-style:none;padding:5px 0;font-size:16px;position:relative;right:20px;}</style>';
					...
					$replace1 = array($inclusion_link, $r_tc , $url1);  
	...
	function StaffTermsofAgreeMent($staff_id){
		...
		//	$SqltermsC = (mysql_query("SELECT * FROM `admin_terms_agreement` WHERE status  = 1"));
			...
			if($countrecords > 0) {
				...
				if($result['company_name'] !='') { $s_company_name = $result['company_name'];}else { $s_company_name = $result['name']; };
				if($result['name'] !='' ) { $s_name = trim($result['name']);}else { $s_name = trim($result['nick_name']); };
				
				$s_abn = $result['abn'];
				$s_bsb = $result['bsb'];
				$s_account_number = $result['account_number'];
				//$gst = $result['staff_gst']; // 1=> Yes 2=>No
				if($result['staff_gst'] == '1') { $s_staff_gst =  "(Yes)"; }else { $s_staff_gst =  "(No)"; }
				...
				if($result['address'] !='') { $s_address = $result['address'];}else { $s_address = ''; };
				$signature = StaffpdfSignature($result['name']);
				
				$search = array('$name','$company_name','$abn','$bsb','$gst','$job_type','$day_avail','$account_number', '$date', '$address','$table','$signature');
				$replace = array($s_name , $s_company_name ,$s_abn,$s_bsb, $s_staff_gst .$gst,$s_job_types,$s_avaibility,$s_account_number , $incExpDate , $s_address,$s_table,$signature);
    ...
    function getJob_accountStatus( $id = null )
    {
		...
		$paymentStatus = mysql_fetch_object(mysql_query("SELECT count(acc_payment_check) as pytStatus FROM `job_details` where job_id = {$id} and acc_payment_check = 0"));
    ...
    { 
	 /*  echo  "SELECT count(pay_staff) as PayStaff FROM `job_details` where job_id = {$id} and ( pay_staff = 1  OR  payment_completed = 1 ) and  status=0";  die; */
	  ...
	  //	print_r($paymentPayStaff); die;
  	    return array('pay_staff'=>$paymentPayStaff->PayStaff,'acc_completed'=>$paymentPayStaff->paymentCompleted);	
	}
			   ...
			   // echo $totalpayment;
		        ...
		        if($flag == 1) {	
		            ...
		            $content .= "<tr class='table_cells ".$color."'>
                            <td class='table_cells'>".changeDateFormate($row['date'],'datetime')."</td>
                            <td class='table_cells'>".$row['payment_method']."</td>	
	...
	function getJobType ($quoteID) {
        ...
        $substaff_mobile =  get_rs_value("sub_staff","mobile",$getData['sub_staff_id']);
		 if($substaff_name != ''){
            ...
            $onchng = "onchange=\"javascript:assing_jobs('".$getData['id']."','".$getData['job_id']."','staff_id_".$getData['id']."');\" style=\"font-size: 12px;\"";
			 ...
			 if($staff_assign_date <= date('Y-m-d H:i:s')) { $style = 'alert_danger_tr';}
	 ...
	 function getApplicationStatus($statusId){
	     $getstatus = array(1=>'Logged',2=>'App',3=>'Doc',4=>'Submitted');
	...
	function get3cxdetailsBydate($call_date){
     ...
     //echo "SELECT count(id) as TotalQuote FROM `quote_new` where 1 = 1 AND  date = '".$date."'";		
		$TotalReco = mysql_fetch_array(mysql_query("SELECT count(id) as TotalQuote FROM `quote_new` where 1 = 1  AND date = '".$date."' AND step not in (7) "));
		$data['TotalQuote'] = $TotalReco['TotalQuote'];
		
		$TotalbookReco = mysql_fetch_array(mysql_query("SELECT count(id) as TotalbookingQuote FROM `quote_new` where 1 = 1 AND   booking_id != 0 AND date = '".$date."' "));
		$data['Totalbookquote'] = $TotalbookReco['TotalbookingQuote'];
		
		$TotaldeletedR = mysql_fetch_array(mysql_query("SELECT count(id) as deletedquote FROM `quote_new` where 1 = 1 AND  deleted = 1 AND date = '".$date."' AND step not in (7) "));
	 ...
	 function getQuoteDetailsStatusdata($date,$siteID) {
        $data = array();	
     //echo "SELECT count(id) as TotalQuote FROM `quote_new` where 1 = 1 AND  date = '".$date."'";	
       $Totalquote  = "SELECT count(id) as TotalQuote FROM `quote_new` where 1 = 1  AND date = '".$date."' AND step not in (7)";	
		  if($siteID != '') {
		...
		// Booking Quote Data
		$bookingSql = "SELECT count(id) as Totalbookquote FROM `quote_new` where 1 = 1 AND   booking_id != 0 AND date = '".$date."' AND step not in (7)";
		 if($siteID != '') {
	...
	function getQuoteByrefrence($date = null,$ref = null,$type = null,$con = null){
		$Sql = "SELECT count(id) as TotalrefRecord FROM `quote_new` where 1 = 1  AND date = '".$date."' AND step not in (7)";
...
function getRefIcons($jobref){
	...
	function getTotalRecordHourly($date, $date_to, $h){
		...
		$countSqtotalQuote = mysql_fetch_array(mysql_query("SELECT count(id) as totalQuote FROM `quote_new` where ( HOUR(createdOn) BETWEEN $h AND $h ) AND date BETWEEN '".$date."' AND '".$date_to."' AND step not in (7)"));
 ...
 function checkPayStaff($job_id){
	...
	function get_totalResultStatus($staff_id, $status) {
	    if($staff_id != 0) {
			...
			$sql = mysql_query("select * from job_details where status != 2 AND staff_id = ".$staff_id." AND job_date  BETWEEN '".date('Y-m-d', strtotime('-7 days'))."' AND  '".date('Y-m-d', strtotime('-1 days'))."' AND job_id in (Select id from jobs where status = ".$status.") GROUP BY job_id");  
	...
	function getquotebyloginID($login_id , $date , $type){
		// for type 1= > Quote , 2=> Booking  
			...
			if($type == 1) {
				$sql = "SELECT *  FROM `quote_new` WHERE 1= 1 AND `login_id` = ".$login_id." AND date = '".$date."'";
			}elseif($type == 2){
				$sql = "SELECT *  FROM `quote_new` WHERE 1= 1 AND `login_id` = ".$login_id." AND date = '".$date."' AND booking_id != '0'";
			}
			$sql .= " AND step not in (1) ";
	...
	function getquoteConverttobyloginID($login_id , $date , $type){
		// for type 1= > Quote , 2=> Booking  
			...
			if($type == 1) {
				...
				$sql = "SELECT *  FROM `quote_new` WHERE 1= 1 AND `login_id` = ".$login_id." AND quote_to_job_date = '".$date."' AND booking_id != '0'";
			}
			$sql .= " AND step not in (1) ";
	...
	function GetDrivingDistance($lat1, $lat2, $long1, $long2)
	{
		...
		return array('distance' => $dist, 'time' => $timechange);
	} 
	...
	function getLatLong($address){
		...
		if($address != ''){
			...
			$data['suber'] = $output->results[0]->address_components[1]->long_name;
			$data['postcode'] = $output->results[0]->address_components[5]->long_name;
	...
	function Calculategetdisctance($from_address , $toaddress)
	{
		 ...
		 $api = file_get_contents("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=".$from_address."&destinations=".$toaddress."&key=AIzaSyAQ3Adlxq9qEPYsKQ2cIjyuIeu04GviYGo");
          ...
          $data = json_decode($api, true);
			...
			$dist = ((int)$data->rows[0]->elements[0]->distance->value / 1000).' Km'; 
			$timechange =  $data->rows[0]->elements[0]->duration->text;
	...
	function gettotalDIstanceCount($quote_id){
		 ...
		 // For Calculate Distance &  time
			...
			return array('travelling_hr'=> $travelling_hr , 'is_flour_from' => $getQuotedetails['is_flour_from'] , 'door_distance_from' => $getQuotedetails['door_distance_from'] , 'is_flour_to' => $getQuotedetails['is_flour_to'] , 'door_distance_to' => $getQuotedetails['door_distance_to'], 'depot_to_job_time' => $depot_to_job_time);
	}
...
function get_cubic_meter_by_position($inventory_type_id ,$type ,$quote_id , $item_pos){
			    ...
			    if(mysql_num_rows($other_types) > 0) {
					...
					while($r = mysql_fetch_assoc($other_types)){
						...
						//echo '<pre>'; print_r($r);
							...
							if($i%2 == 0) { $color = 'background:#f1f1f1 ;padding-top: 5px;padding-bottom: 5px;padding-left:10px; font-weight:300;font-size: 16px;'; }else { $color = 'background:#FFF ;padding-top: 5px;padding-bottom: 5px;padding-left:10px; font-weight:300;font-size: 16px;';}
  ...
  ini_set('display_errors', TRUE); */
	        ...
	        if($getstaffdetails != '') {
				  ...
				  $checkInvoice = mysql_query("SELECT * FROM `franchise_report` WHERE staff_id = ".$staff_id." AND date_name='".strtolower(date('F' , strtotime($from_date)))."' AND year='".date('Y',strtotime($from_date))."'");
					...
					if(mysql_num_rows($checkInvoice) == 0) {	
						 ...
						 mysql_query("INSERT INTO `franchise_report` (`email`, `staff_id`, `invoice_number`,  `invoice_from_date`, `invoice_to_date` , `date_name` , `year`) VALUES ('".base64_encode($content)."', '".$staff_id."','".$totalinvoice."', '".$from_date."' , '".$todate."' , '".strtolower(date('F' , strtotime($from_date)))."' , '".date('Y',strtotime($from_date))."');"); 
					} 
	...
	function getrecleanRecords($staff_id , $from_date ,$todate ,$type)
	{
		...
		return array('jobreclean'=>$jobreclean, 'pecentrecl'=>number_format($pecentrecl,2) ,  'tofailed_re'=>$tofailed_re, 'tillrecleantilldate'=>number_format($tillrecleantilldate,2) , 'totaljobdone' =>$totaljobs);
	}
	...
	function getStaffRosterDetails($getdate , $staffid ,$status ,  $todate = null)
	{
        ...
        function CheckAvailORnNOT($from_date ,$todate , $staffid){
            ...
            return array('avail'=>$avail , 'notavail'=>$notavail);
	...
	function getTotalBBCAmountDetails($staff_id , $from_date ,$todate ,$type){
		...
		return array('totalamount'=>$getdata['totalamount'] , 'totalstaff'=>$getdata['totalstaff']);
   ...
   function sendNotiMessage($str , $result){
        /* $getlogin_device = mysql_fetch_assoc(mysql_query("SELECT GROUP_CONCAT(device_id) as deviceid  FROM `login_device` WHERE is_logged_in = 1 AND device_id != '' AND login_id = '".$result['chatter']."'")); */
		  ...
		  $totalcount  = mysql_num_rows($sql);
		    if($totalcount > 0) {
			   while($getjob = mysql_fetch_assoc($sql)) {
			    ...
			    return array('total_job'=>$totalcount , 'jobs'=>$getjob1);
		    }else{
		      return array('total_job'=>0 , 'jobs'=>'No Job Found');
			}
	} 
			...
			if($countresult > 0) {
			    while($getdata = mysql_fetch_assoc($cl_id)) {
				   ...
				   $getjob1[] = $getdata['job_id'];
			        ...
			        return array('jobtotal'=>$countresult ,  'jobs'=>$getjob1);
			}
			 
		 }else{
			return array('jobtotal'=>$countresult ,  'jobs'=>'not');
		 }
	}
	...
	function makejobTimeLine($job_id){
		...
		if($count_n > 0 || $count_e > 0) {
				...
				$str  = '<html><body>';
			    $str  .= '<h2>Job  Communication Time Line</h2>';
			    $str .= '<div class="container mt-5 mb-5">
				   ...
				   $str .='<ul style="list-style-type: none;position: relative;padding-left: 30px;">'; 
					 ...
					 foreach($fullgetdata as $key=>$value) {
							...
							if($value['comment'] != $value['heading']) { if($value['comment'] != '') { $commentdata =  $value['comment']; }else { $commentdata =  $value['heading']; }}
                          $site_url =   get_rs_value("siteprefs","site_url",1);
					      $icon = $site_url.'/admin/images/li_icon.png';
					  
					       if($value['type'] == 'email') { $sendemailon = '<span style="font-size:14px; padding-left: 5px;" >('.$value['email'].')</span>';  }
			...
			//	$send_emailto = "payal@bcic.com.au";
				...
				echo '<span style="color: #067706;float: right;margin-right: 137px;">Send Email Successfully</span>';
	...
	function get_total_work_hr($job_id ,$q_hr){	
		...
		$extra_hr = 0;
		 ...
		 return array('work_hr' => $work_hr , 'total_work' => $total_work , 'team_size'=>$team_size , 'extra_hr' =>$extra_hr);
    }
...
function StaffDetailsUpdateByadmin($oldDetails,$newDetails) 
	{
		...
		if(!empty($result1)) 
		{
			...
			sendmailbcic("BCIC Staff Details changes",'payal@bcic.com.au',$subject,$body,'hr@bcic.com.au',"0");
...
function getTotalrecord($arg) {
	...
	function checkSignatureImg($jobid){
	  ...
	  function getTotalQuoteInfo($fromdate  ,$todate , $site_id = null){
		...
		$sql =   "SELECT * FROM `quote_new` WHERE date >= '".$fromdate."' AND date <= '".$todate."'";
		 ...
		 return array('totalquote'=>$count, 'booked'=>$booked,'site'=>$site,'adminquote'=>$adminquote,'is_deleted'=>$is_deleted , 'phone'=>$phone , 'totalc_denied'=>$totalc_denied , 'totala_denied'=>$totala_denied , 'totala_other'=>$totala_other); 
	  }	 
	...
	function quoteBySite($fromdate  ,$todate , $typeid , $type){
	 ...
	 return array('totalquote'=>$count , 'totalbooking'=>$totalbooing);
    }	
	...
	function getmonthlyQuote($month, $type){
		...
		$sql = "SELECT id FROM `quote_new` WHERE  date >='".$fromdate."'  AND date <= '".$todate."'";
		if($type == 1) {
		 $sql.= " AND booking_id  != 0";
		}
	 ...
	 function gettotalAmountData($fromdata ,  $todata ,$jobtypeid){
		...
		return array('totalamt'=>round($data['totalamt'] , 0) ,'staffamt'=>round($data['staffamt'] , 0) ,'profitamt'=>round($data['profitamt'], 0));
	} 
  ...
  function getjobStatusData($fromdata ,  $todata ,$statusid , $site_id = null){
	...
	function getTotalJobsDetails($fromdate , $todate ,$siteid){
		...
		// Job Cancelled	
			...
			$jobs_booked_data_sql = "SELECT * FROM `quote_new` WHERE booking_id!=0  AND  date>='".date('Y-m-d' , strtotime($fromdate))."' and date<='".$todate."' and site_id=".$siteid."";
			...
			return array('jobs_phone'=>$jobs_Phone , 'jobs_site'=>$jobs_Site , 'total_jobs'=>$total_jobs, 'jobs_cancelled'=>$jobs_cancelled);
	}
	...
	function  checkCountFailedreclean($arg) {
	    ...
	    if (mysql_num_rows($data)>0){ 
					  ...
					  // echo "select * from job_reclean where job_id=".$row['job_id']." and status!=2 AND reclean_work = 2";
						...
						$argSql  = mysql_query("select * from job_reclean where job_id=".$row['job_id']." and status!=2 AND reclean_work = 2");	
		  ...
		  // Update job SMS
			...
			if($job_details['job_type_id'] != "11"){
		    	...
		    	if($staff['better_franchisee'] != 2 && $staff['payment_type'] != 1) {
				   ...
				   //$comment.= "  for Amount of $".$job_details['amount_total']." ";
				   $comment.= " Amount of $".$job_details['amount_total']." ";
				}else{
				   $comment.= '';	
				}
			}
			...
			if($staff['better_franchisee'] != 2 && $staff['payment_type'] != 1) {	
				...
				$comment.=" $".$job_details['amount_total'];	
			}
...
//     }
	...
	function checkExtraJob($jobid) {
		...
		return array('checkf'=>$checkf , 'extraamount'=>$extraamount);
	}
    ...
    function system_dd_type($typeid){
	   $sql = mysql_query("select id , name from system_dd where type = ".$typeid." Order by id asc");
	   while($data = mysql_fetch_assoc($sql)) {
	...
	function dd_value($type){
	   $sql = mysql_query("select id , name   from system_dd where type = ".$type."");
	   while($data = mysql_fetch_assoc($sql)) {
	   ...
	   return $getvalue;
		...
		//	$apiKey = 'AIzaSyAQ3Adlxq9qEPYsKQ2cIjyuIeu04GviYGo';
			...
			// Geocoding API request with start address
			 $url = "https://maps.googleapis.com/maps/api/directions/json?origin=".$formattedAddrFrom."&destination=".$formattedAddrTo."&key=".$apiKey."&sensor=false";
	            $ch = curl_init();
	            curl_setopt($ch, CURLOPT_URL, $url);
	            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	            curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
	           ...
	           // print_r($response_all);
	            ...
	            // print_r($response);
	              ...
	              $time = $response_all->routes[0]->legs[0]->duration->text;
	...
	function getTaskReCord($loginid , $stageskey, $fromdate , $todate ,$type, $data =null){
	...
	function getTodayOverDueTaskReCord($loginid , $fromdate , $todate ,$type){
	...
	function send_voucher_emailsdata($qdata, $RandomString){
		...
		if(!empty($qdata)) {
		    ...
		    $str = 'Hi '.$qdata['name'].','.$eol.''.$eol.'
					...
					Thank you so much for your recent feedback!'.$eol.''.$eol.' 
					We hope that your move went well and you’re settling into your new home.'.$eol.''.$eol.' 
					We cannot emphasize how important is it for us to get your feedback. '.$eol.' '.$eol.'

					its just the nature of business '.$eol.' 
					“Good work hardly gets appreciated, but 1 small mistake out of 100 jobs we do every week gets highlighted on social media”. '.$eol.' 

					We just want people to know that we do good work too. '.$eol.' '.$eol.'
					Currently, we are running a special promotion for all the good people like yourself who are happy with our work. Please if you can simply copy and paste your original review on https://www.productreview.com.au/listings/bcic and give us 5 Stars!'.$eol.' '.$eol.'

					<h4><strong>We will send you a <span style="color: red;">$50 Thank You Gift Voucheras</span> our token of appreciation!</strong></h4>'.$eol.' 
					<strong>Positive Feedback </strong>'.$eol.'';

					if($qdata['positive_comment'] != '') {
							$str .= '<table border="1">
							<tr><td style="padding:5px; border: 1px solid;">'.$qdata['positive_comment'].'</td></tr>
								  </table>'.$eol.$eol.'';		
					}
						
			//$str .= 'Your Coupon Code :   <strong>' .$RandomString .'</strong>'.$eol.$eol.'';		
		    $str  .= 	' It takes less than 5 minutes to leave a 5 Star Review on Google or Product Review, these Reviews are so important for the survival of a local business.'.$eol.' 

					Please, we cannot thank you enough for your time and appreciation. '.$eol.' 
					https://www.productreview.com.au/listings/bcic'.$eol.' 
	...
	function get_sales_activity($qid , $tasktype = 1){
		$sql = mysql_query("SELECT * FROM `task_manager`  WHERE quote_id = ".$qid." AND task_type = ".$tasktype." AND admin_id != 0 ORDER BY `id` DESC");
		$arrdata = array();
		   ...
		   if($jobtypeid != 0) {
            ...
            if($stageskey == 0) { 
			  // Total
				...
				$argsql1 .=" AND job_details.job_id  IN (SELECT DISTINCT(job_id) as jobid FROM `job_befor_after_image` WHERE  image_status IN (5) AND job_type_status = 1) ";
		    }elseif($stageskey == 6) {
	...
	function getAfterJobs_data($stageskey, $adminid = 0 , $pagetype = 0)
	{
		...
		if($stageskey == 1) {
		 ...
		 // $arg .=" AND status = 1 AND job_date <= '".$date."' AND id in (SELECT job_id FROM `job_details` where pay_staff = 0 AND   status != 2 AND ( end_time != '0000-00-00 00:00:00' OR start_time != '0000-00-00 00:00:00' )) ";
		  $arg .=" AND status = 1 AND job_date < '".$date."' AND id in (SELECT job_id FROM `job_details` where pay_staff = 0 AND   status != 2 ) ";
		}elseif($stageskey == 2) {
         ...
         $arg .=" AND status = 1 AND job_date < '".$date."' AND id in (SELECT  DISTINCT(job_id) as jobid FROM `client_image_befor_after` WHERE job_type = 1 and img_type = 2)";
          ...
          $arg .=" AND status = 5 AND id in (SELECT DISTINCT(job_id) as jid from job_reclean WHERE status != 2 and reclean_date <= '".$date."' and end_time != '0000-00-00 00:00:00')";
		}elseif($stageskey == 4) {
          ...
          $arg .=" AND status = 4";
            ...
            $arg .=" AND status = 3 and review_email_time != '0000-00-00 00:00:00' AND review_call_done = '0000-00-00 00:00:00' AND job_date <= '".$date."' ";
		}elseif($stageskey == 6) {
	...
	function reclean_data($stageskey, $adminid = 0 ,$pagetype = 0){
		...
		if($stageskey == 1) {
		  ...
		  $arg .=" AND status = 9";
			...
			//•	After Job Images Received: If client uploads after images job will show here 
          $arg .=" AND status = 9 AND awaiting_exit_report != '0000-00-00 00:00:00'"; 
          ...
          $arg .=" AND status = 5 AND id in (SELECT DISTINCT(job_id) as jid from job_reclean WHERE status != 2 AND reclean_status = 1)";
			...
			//•	Reclean Accepted 
          $arg .=" AND status = 5 AND id in (SELECT DISTINCT(job_id) as jid from job_reclean WHERE status != 2 AND reclean_status = 2)";
		}elseif($stageskey == 5) {
			//•	Reclean Started
          $arg .=" AND status = 5 AND id in (SELECT DISTINCT(job_id) as jid from job_reclean WHERE status != 2 AND reclean_status = 3)";
		}elseif($stageskey == 6){
			//•	Day Before Re-clean 
	...
	function getOverallReview($jobid){
		 ...
		 $review_date = $getdata['review_date'];
		 return array('yellow_star'=>$yellow_star, 'review_date'=>$review_date);
	}
	 
		function getReCleanJobs($stageskey , $qid)
		{
			
				if($stageskey == 1 || $stageskey == 2) {
		...
		{	
			...
			$sql = mysql_query($arg);
			$data = mysql_fetch_array($sql);
			return array('count'=>$data['totalCount'] , 'experienceReview'=>$data['experienceReview'] );
		}
 ...
 function GetQuoteDetailsIconData($booking_id, $qid){
		...
		return array('tr_removal'=>$tr_removal, 'icondetais'=>$getdata);
    }	 
	...
	function BBCgetTotalAmountOffered($jobid , $jobtypeid)
	{
		  if(is_array($jobtypeid)) {
		     $job_types = implode(',',$jobtypeid);
              ...
              return array('jobamt'=> $totalamount ,'staffamot'=>$totalstaffamt);   				     
 	}	
	...
	function BBCOfferedJobAmount($staff_id , $fromdate, $todate , $action, $stafftype)
	{
					   ...
					   if($count > 0) {
							   ...
							   while($data = mysql_fetch_assoc($querydata1)){
								       ...
								       $getJobtypeID[$data['job_id']][] = array('jobtype_id'=>$data['job_type_id'] ,'total_amount'=>$data['total_amount'] ,'total_staff_amt'=>$data['total_staff_amt']);
									   
								   /*  if($stafftype == 2 && $action == 2) {
										
									 $getJobtypeID[$data['job_id']][] = array('jobtype_id'=>$data['job_type_id'] ,'total_amount'=>$data['total_amount'] ,'total_staff_amt'=>$data['total_staff_amt']);
										
									}else{
								        $getjobid[] = $data['job_id'];
									    $getJobtypeID[$data['job_id']][] = $data['job_type_id'];
									} */
								}
								
						 /*if($action == 2) {	
echo "<pre>";						 
							print_r($getJobtypeID);	
						} */
								  foreach($getJobtypeID as $jobid1=>$jobtypeid) {
											 $jobidty = array_unique($jobtypeid);
											  if(!empty($jobidty)) {
												  echo $getJobtypeID[$jobid1]['total_amount'];
												  if($action == 2){
													  $totalamt['totaljobamt'][] = $getJobtypeID[$jobid1]['total_amount'];
														$totalamt['totalstaffamt'][] = $getJobtypeID[$jobid1]['total_staff_amt']; 
												  }else{
												   $arr =  BBCgetTotalAmountOffered($jobid1 , $jobidty);
								 ...
								 if($action == 2){
									 $staffamotamout = $totalamout *60/100;
								 }
								
								 
								 return array('jobid'=>$jobid , 'totalamout'=>number_format($totalamout,2) , 'staffamotamout'=>number_format($staffamotamout,2));
								 unset($arr);   
					   }
					   unset($getJobtypeID); 
	...
	function BBCJobsByPostCode($staff_id , $fromdate, $todate , $action, $postcode)
	{	
		$staff_postcode_arr = explode(',',$postcode);
		   $sql1 = "SELECT job_id ,job_type_id   FROM `staff_jobs_status` WHERE staff_id = ".$staff_id."  and status = {$action}  and DATE(created_at) >= '".$fromdate."' AND DATE(created_at) <= '".$todate."'";
		    	$querydata1 = mysql_query($sql1);
		       $count = mysql_num_rows($querydata1);
		   ...
		   if($count > 0) {
									  ...
									  if(!empty($val)) {
											if(in_array($jobid,$postcodejob)){
												    ...
												    foreach($val as $key1=>$jobtypeid) {
														...
														$arr1 =  BBCgetTotalAmountOffered($jobid , $jobtypeid);
				 ...
				 return array('total_job'=>$totaldeniedInpostcode , 'totalamout'=> number_format(array_sum($totaldeniedamt['totaljobamt']),2) , 'staffamout'=>number_format(array_sum($totaldeniedamt['totalstaffamt']),2));	
