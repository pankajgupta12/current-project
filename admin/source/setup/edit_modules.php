...
$insert=insert_form($fields1,$dtype1,"module_details",$dadd1);
$last_id = mysql_insert_id();
	...
	$ins1 = "update module_details set ";
	foreach($fields2_arr as $fname){
...
$arg ="select * from module_details where module_id=".$_GET['id'];
...
var ftvalue ="";
	if(obj.value=="dd"){
		ftvalue = "dropdown_name,table,value_field,name_field,cond,onchng,details";
...
<tr>
    ...
    <td width="85%">
      <? echo create_dropdown("module_id", "modules","id","name"," type=1 and id not in (select module_id from module_details)","",$details);?>    </td>
  </tr>
  <tr>
    ...
    <td width="85%">
      ...
      <?php
		...
		while ($row = mysql_fetch_row($result)) {
			if ($details['table']==$row[0]){ $selected = "selected"; }else{ $selected=""; } 
...
<? if ($details['table']!=""){ ?>
        ...
        <tr>
            ...
            <td width="85%"><select name="folder" id="folder">
              <option value="try" <? if($details['folder']=="try"){ echo "Selected"; ?> >try</option>
              <option value="manage" <? if($details['folder']=="manage"){ echo "Selected"; ?> >manage</option>
            </select></td>
          </tr>
          <tr>
            <td width="15%">Title</td>
            <td width="85%"><input name="title" type="text" value="<? echo $details['title'];?>" /></td>
          </tr>
          <tr>
            <td>Image Folder</td>
            <td><input name="image_folder" type="text"  value="<? echo $details['image_folder'];?>"/></td>
          </tr>
          <tr>
            <td>List Name</td>
            <td><input name="listname" type="text" id="listname" size="150" value="<? echo $details['listname'];?>" /></td>
          </tr>
          <tr>
            <td>List Heading</td>
            <td><input name="listheading" type="text" id="listheading" size="150" value="<? echo $details['listheading'];?>" /></td>
          </tr>
        </table>
        ...
        $arg1 = "select * from ".$details['table']."";
        ...
        ?>
          <tr>
            ...
            <td align="center"><? echo create_dropdown("f_type_".$i."", "module_field_type","name","name"," 1 ","onchange=\"javascript:type_value(this,'f_type_value_".$i."');\"",$details);?></td>
            <td align="center"><input name="f_type_value_<? echo $i ?>" type="text" id="f_type_value_<? echo $i ?>" size="55"/></td>
            <td align="center"><input name="valid_<? echo $i ?>" type="checkbox" id="valid_<? echo $i ?>" checked="checked" /></td>
