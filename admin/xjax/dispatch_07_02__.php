<?php 
   include($_SERVER['DOCUMENT_ROOT'].'/admin/source/functions/functions_new_pankaj.php');
	...
	$staff_arg = "select id, name , email ,site_id ,job_types, mobile , nick_name, team_of , staff_member_rating , no_work from staff where status=1";
	...
	$countResult = mysql_num_rows($staff_table);
	    ...
	    $getjobSql =   mysql_query("SELECT id , job_icon , name,short_name FROM `job_type`");
                ...
                $getStaffData = getStaffOnDispatch();	
                $getSubStaff = getSubStaff_data();	
				
				//print_r($getStaffData); die;
		echo date('H:i:s');	
	?>
	<div class="table-scroll">
		...
		<tr>
			<td>
				<div class="td_back" <?php if($staff['no_work'] == '2') {  ?> style="background: #f7baba;" title="Not Work" <?php }   ?> ><strong><?php if($staff['nick_name'] &&  $staff['nick_name'] !='') { echo $staff['nick_name']; }else {echo $staff['name'];}?></strong><br>
			...
			for($i=1;$i<=7;$i++)
			{
				...
				if(mysql_num_rows($job_details_data)>0){
					...
					{	    
				        ...
				        $getJobdata =   getJobDetailsInDishpatch($job_details['job_id']);
				    
					  // print_r($getJobdata);
					   // print_r($getJobdata); die;
				
						// $job_status = get_rs_value("jobs","status",$job_details['job_id']);	
						 $job_status = $getJobdata['status'];	
						$cehcek_payment_status = checkAmountPay($job_details['job_id']);
						
						$payment_status = $cehcek_payment_status['payment_status'];
						...
						$job_details_amt = mysql_fetch_array($job_details_data1);
  					    ...
  					    if($job_payment['total_amt']==""){ 
					       ...
					       /*  $getQuote = mysql_fetch_array(mysql_query("Select name,suburb from quote_new where id = ".$job_details['quote_id']));
							
							
							$q_name   = $getQuote['name'];
							$q_suburb   = $getQuote['suburb']; */
							
							$q_name   = $getJobdata['name'];
							$q_suburb   = $getJobdata['suburb'];
							...
							if($job_status=="5"){ 
								  ...
								  $argSql  = "select quote_id,staff_id,job_type_id , sub_staff_id from job_reclean where job_id=".$job_details['job_id']." and status!=2";		
	 
							}else { 
								  $argSql  = "select quote_id,staff_id,job_type_id , sub_staff_id from job_details where job_id=".$job_details['job_id']." and status!=2";							
								   $page = 'jobs';
								   $type = '';
								   $tol_type = 'Job';
							} 
						 ...
						 $reclean_job = $cehcek_payment_status['recleanjobs'];
							
					  ?>
					 
						<div class="td_back td_back_<?php if($reclean_job == "2" && $job_status == 3) { echo 're_dark'; }elseif($job_status=="3" && $reclean_job == 1){ echo 'green'; }elseif($job_status=="1"){ echo 'red'; }else if($job_status=="4"){ echo 'orange'; }else if($job_status=="5"){ echo 'darkred'; }  ?>">
								 ...
								 //echo $argSql;
								  ...
								  if(mysql_num_rows($job_details1)>0) {
										...
										$job_icon =  $jobtype[$jdetails['job_type_id']];
										   ...
										   $staffdetails   = $getStaffData[$jdetails['staff_id']];
										 ...
										 if($jdetails['staff_id'] != 0) {
											...
											if($jdetails['sub_staff_id'] != 0) {
                                              // $substaffdetails = mysql_fetch_array(mysql_query("Select mobile,name from sub_staff where id = ".$jdetails['sub_staff_id']));
											   $substaffdetails = $getSubStaff[$jdetails['sub_staff_id']];
