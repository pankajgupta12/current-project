...
<?
  ...
  //print_r($_SESSION['view_report_list']);
      $resultsPerPage = resultsPerPage;

	if(!isset($_SESSION['view_report_list']['import_id'])){ $_SESSION['view_report_list']['import_id'] = $_GET['import_id']; }
	$cond = '';
	$arg = "select * from c3cx_calls 	WHERE 1=1";	
	
	if($_SESSION['view_report_list']['import_id'] != '') {
		$arg .= " AND c3cx_calls.import_id = '".$_SESSION['view_report_list']['import_id']."'";
		//$cond .= " AND c3cx_calls.import_id = '".$_SESSION['view_report_list']['import_id']."'";
	}
	
	if($_SESSION['view_report_list']['admin'] != '' && $_SESSION['view_report_list']['admin'] != '0') {
	 $arg .= " AND c3cx_calls.from_type = '".$_SESSION['view_report_list']['admin']."'";
	 //$cond .= " AND c3cx_calls.from_type = '".$_SESSION['view_report_list']['admin']."'";
	}
	if($_SESSION['view_report_list']['quote_job_id'] != '' && $_SESSION['view_report_list']['quote_job_id'] != '0') {
	  $arg .= " AND ( c3cx_calls.quote_id = '".$_SESSION['view_report_list']['quote_job_id']."' OR c3cx_calls.job_id = '".$_SESSION['view_report_list']['quote_job_id']."' )";
	  //$cond .= " AND ( c3cx_calls.quote_id = '".$_SESSION['view_report_list']['quote_job_id']."' OR c3cx_calls.job_id = '".$_SESSION['view_report_list']['quote_job_id']."' )";
	}
	$condition =  $arg;
	$arg .=" ORDER BY c3cx_calls.id DESC     	LIMIT 0, $resultsPerPage"; 
	
	$totalCount =   mysql_num_rows(mysql_query($condition));
	
	//echo $arg;
	$arg;		
	$query = mysql_query($arg);//rows count//
	...
	<div class="right_text_box" style="margin-top: -42px;">
				<div class="midle_staff_box"> <span class="midle_text">Records Found <?php echo $totalCount; ?> </span></div>
			</div>
            ...
            <tbody id="get_loadmoredata">
                ...
                ?>
                    <tr class="parent_tr <?php echo $bgcolor; ?>" id="deletecall_import_<? echo $r['id'];?>">
                        ...
                        <!-- call date -->
                        <td class="bc_click_btn">
                            <? echo changeDateFormate($r['call_date'],'datetime');?>
                        </td>
                        <!-- call time -->
                        <td class="bc_click_btn">
                            <? echo $r['call_time'];?>
                        ...
                        <!-- to number -->
                        <td class="bc_click_btn  <?php  echo $class;?>">
                            <? echo $r['ivr_option'];?>
                        </td>
                        <td class="bc_click_btn">
                            <? echo $r['terminated_by'];?>
                        </td>
                        <!-- duration -->
                        <td class="bc_click_btn">
                            <? if( $r['duration'] != '' ){echo $r['duration'];}else{ echo '--'; }?>
                        </td>
                        <td class="bc_click_btn">
