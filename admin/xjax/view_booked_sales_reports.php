...
if(!isset($_SESSION['sales']['to_date'])){ $_SESSION['sales']['to_date'] = date("Y-m-t"); }
   ...
   $arg =  mysql_query("SELECT name , id  FROM `admin` WHERE `is_call_allow` = 1 AND status != 0");
     ...
     $getSql =  "SELECT DISTINCT(date) as date FROM `quote_new`  where 1=  1 ";
	  ...
	  if(isset($_SESSION['sales']['from_date']) && $_SESSION['sales']['to_date'] != '' )
		{ 
...
?>
   <span class="staff_text" style="margin-bottom: -11px;margin-left: 15px;">Sales Reports A/C to Quote Convert Date</span>
<br/><br/>
			...
			?>  
				<tr class="table_cells">
				 ...
				 <?php foreach($getadmin as $key=>$name) {
                       $getquote1 =     getquoteConverttobyloginID($key , $getquote['date'] , 1);
                       $getbooked1 =     getquoteConverttobyloginID($key , $getquote['date'] , 2);
				...
				$getnonloginq =  getquoteConverttobyloginID(0 , $getquote['date'] , 1);
				$getnonloginb =  getquoteConverttobyloginID(0 , $getquote['date'] , 2);
				 ?>
