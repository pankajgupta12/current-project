<?
	...
	if(!isset($_SESSION['reclean']['status'])){ $_SESSION['reclean']['status'] = 5; }
		...
		$arg = "SELECT * FROM `job_details` WHERE status != 2 and job_id in (SELECT id from jobs WHERE status = 5)";
...
?>
		...
		<div class="right_text_box" >
		    <div class="midle_staff_box"> <span class="midle_text">Total Records Found <?php echo checkCountFailedreclean($arg); ?> </span></div>
		</div>
