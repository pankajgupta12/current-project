 ...
 session_start();
  ...
  include("../../source/functions/config.php");
   if(isset($_POST['page'])) {	
    ...
    // echo  'xxxxxxxxxxxxxxxxx' .$arg;  die;
...
?> 
				 ...
				 <td class="bc_click_btn pick_row" title="<? if($r['first_name']){ echo $r['first_name']; }else { echo  $r['given_name']; }?>"><? if($r['first_name']) { echo $r['first_name']; }else { echo  $r['given_name']; }?></td>
					...
					<td class=" pick_row" id="step_status_<?php echo $r['id']; ?>"><?php echo create_dd("step_status","system_dd","id","name","type=55","onchange=\"javascript:edit_fields_applications(this,'staff_applications.step_status',".$r['id'].");\"",$r);?> </td>
