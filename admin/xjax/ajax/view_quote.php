...
$arg = "select * from quote_new where 1=1 AND step != 10";
...
?>
     ...
     <div class="right_text_boxData right_text_box">
	  <div class="midle_staff_box"><span class="midle_text">Total Records <?php  echo $countResult; //echo  getTotalRecords('quote_new',$condcount); ?></span></div>
	</div>
    ...
    <table class="user-table">
	  ...
	  while($r=mysql_fetch_assoc($data)){
		  if($r['booking_id'] == '0') {
			...
			$sql_icone = ("select job_type_id , job_type from job_details where  status != 2 AND quote_id=".$r['id']);  
		  }
		    $quote_details = mysql_query($sql_icone);
		  
		  //added to use area code before number for bcic removal
		  if($r['booking_id'] == '0') {
			$removal_quote = ("select * from quote_details where  status != 2 AND job_type_id='11' AND quote_id=".$r['id']);
		  }else{
			$removal_quote = ("select * from job_details where  status != 2 AND job_type_id='11' AND  quote_id=".$r['id']);  
		  }
	  ...
	  ?>
		...
		<tr class="parent_tr <?php if($r['booking_id'] != 0 ) { echo " alert_danger_success";} else if($r['step'] == 5 || $r['step'] == 6 || $r['step'] == 7 || $r['deleted'] == 1) { echo ' alert_danger_tr'; }else if($r['response'] == '3') { echo 'alert_warning'; }?> ">
			...
			<td class="" ><? echo getSystemvalueByID($r['have_removal'],90);?></td>	
...
<script>
	document.querySelector(".usertable-overflow").addEventListener("scroll",function(e){
		
		this.querySelector(".myTable").style.top = this.scrollTop + "px";
