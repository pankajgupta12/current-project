<?php 
	...
	$staff_arg = "select id, name ,do_not_call, email ,site_id ,job_types, mobile ,(SELECT name FROM sites WHERE sites.id = staff.site_id) as sname ,  (SELECT name  FROM `system_dd` WHERE `type` = 48 AND system_dd.id = staff.team_of) as teamof ,  nick_name,  staff_member_rating , no_work from staff where status=1";
		...
		<tr>
				...
				<div class="td_back" <?php if($staff['do_not_call'] == '2') { ?>  style="background: #ebde71;" title="DO Not Call"   <?php  }else if($staff['no_work'] == '2') {  ?> style="background: #f7baba;" title="Not Work" <?php }   ?> ><strong><?php if($staff['nick_name'] &&  $staff['nick_name'] !='') { echo $staff['nick_name']; }else {echo $staff['name'];}?></strong><br>
				   <span class="text11">(<?php echo $staff_location;?>)  (<?php echo $job_type;?>)</span>
					<div class="tooltip_div">
						...
						<span class="name_text">Team of : <?php  echo  $staff['teamof'];  //echo getSystemvalueByID($staff['team_of'],48);?> </span>
		  ...
		  <?php
			...
			for($i=1;$i<=7;$i++)
			{
				...
				$jd_arg = "select job_id,quote_id , reclean_job ,  (SELECT CONCAT(name,' __ ',suburb,' __ ', bbcapp_staff_id, ' __ ', booking_date,' __ ', date) from quote_new WHERE quote_new.id = job_details.quote_id) as quoteInfo from job_details where status!=2 ";
				...
				$jd_arg.= " and job_id in (select id from jobs where status in(1,3,4,5)) OR job_id in (select job_id from job_reclean where job_reclean.status!=2 and job_reclean.staff_id=".$staff['id']."";
				$jd_arg.= " and job_reclean.reclean_date='".date("Y-m-d", $sdate_staff)."')";
				
				
				$jd_arg.= "  GROUP by job_id ORDER by job_type_id ASC";
				...
				if(mysql_num_rows($job_details_data)>0){
					...
					{	    
						...
						$job_details_data1 = mysql_query("SELECT sum(amount_total) as total_amt FROM `job_details` WHERE  job_id=".mysql_real_escape_string($job_details['job_id'])." and status!=2");
						$job_details_amt = mysql_fetch_array($job_details_data1);
  					    ...
  					    if($job_payment['total_amt']==""){ 
					       ...
					       // $getQuote = mysql_fetch_array(mysql_query("Select name,suburb, bbcapp_staff_id,booking_date,date from quote_new where id = ".$job_details['quote_id']));
							$getQuote = explode('__' , $job_details['quoteInfo']);
                 ...
                 //print_r($getQuote);						
							...
							$q_name   = $getQuote[0];
							$q_suburb   = $getQuote[1];
							$bbcapp_staff_id   = $getQuote[2];
							$booking_date   = $getQuote[3];
							$date   = $getQuote[4];
							...
							if($job_status=="5"){ 
								  ...
								  $argSql  = "select quote_id,staff_id,job_type_id , sub_staff_id , (Select CONCAT( name , ' ___ ' ,mobile) from staff where staff.id = job_reclean.staff_id ) as staffinfo, (Select description from quote_details where quote_details.quote_id = job_reclean.quote_id AND job_type_id = job_reclean.job_type_id ) as quoteDesc from job_reclean where job_id=".$job_details['job_id']." and status!=2";		
	 
							}else { 
								  $argSql  = "select quote_id,staff_id,job_type_id , sub_staff_id , (Select CONCAT( name , ' ___ ' ,mobile) from staff where staff.id = job_details.staff_id ) as staffinfo, (Select description from quote_details where quote_details.quote_id = job_details.quote_id AND job_type_id = job_details.job_type_id ) as quoteDesc from job_details where job_id=".$job_details['job_id']." and status!=2";							
								   $page = 'jobs';
								   $type = '';
								   $tol_type = 'Job';
							} 
						...
						//echo $argSql	
						 // $reclean_job = get_sql("job_details","reclean_job"," where job_id = '".$job_details['job_id']."'  and job_type_id = '1' AND status != 2");
						  $reclean_job = $job_details['reclean_job'];
							
					  ?>
					 
						<div <?php  if(($bbcapp_staff_id == $staff['id']) && $date == date('Y-m-d')) {  ?> style="background: #7bd67f;"  <?php  } else { ?>   class="td_back td_back_<?php if($reclean_job == "2" && $job_status == 3) { echo 're_dark'; }elseif($job_status=="3" && $reclean_job == 1){ echo 'green'; }elseif($job_status=="1"){ echo 'red'; }else if($job_status=="4"){ echo 'orange'; }else if($job_status=="5"){ echo 'darkred'; } } ?>">
							...
							<?php  
								 ...
								 //echo $argSql;
								  ...
								  if(mysql_num_rows($job_details1)>0) {
										...
										$staffdetails = explode('___' ,$jdetails['staffinfo']);
										
										$staff_name  = $staffdetails[0];
										$staff_mobile  = $staffdetails[1];
										 ...
										 if($jdetails['staff_id'] != 0) {
											 ...
											 $get_desc = $jdetails['quoteDesc'];
											 
											if($jdetails['sub_staff_id'] != 0 && $jdetails['sub_staff_id'] > 0) {
